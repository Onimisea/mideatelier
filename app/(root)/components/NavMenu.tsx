...
export function NavMenu() {
  ...
  //       router.push("/job-board/my-applications");
  //     },
  //     onError: (error) => {
  //       console.log(error);
  ...
  //           // Iterate over field -> messages
  //           Object.entries(axiosError.response.data.errors).forEach(
  //             ([messages]) => {
  //               if (Array.isArray(messages)) {
  //                 messages.forEach((msg) => {
  //                   toast.error(msg); // show each validation message
  //                 });
      ...
      <motion.div variants={itemVariants}>
        <Dialog>
          <DialogTrigger asChild>
            <Button className="bg-muted-gold hover:bg-transparent text-black hover:text-muted-gold hover:border hover:border-muted-gold px-10 py-6 text-lg rounded-full shadow-lg transition-all duration-300 hover:shadow-muted-gold/50 ">
              Book Now
            </Button>
          </DialogTrigger>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex items-start space-x-2">
                          ...
                          <FormLabel className="text-sm font-normal text-deep-navy">
                            I understand that consultations are by appointment
